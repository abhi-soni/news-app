...
const News = (props) => {
  ...
  //         try {
  //             const response = await axios.get('https://news-app-wkxd.onrender.comco', apiOptions);
  //             setArticles(response.data.articles);
  //             setLoading(false);
  //         } catch (error) {
  ...
  return (
    <div className="container my-3">
      <h2 className="text-center" style={{ marginTop: "68px" }}>
        Latest News
      </h2>
        ...
        <div className="row">
          {!loading &&
            ...
            articles.map((element) => {
              // here, elements represents "articles" object
              // map function will render(show) <div> tag times equal to number of objects inside "articles" object
              return (
                <div key={element.url} className="col-md-4 col-sm-12">
                  <NewsItem
                    title={
                      element.title
                        ? element.title?.slice(0, 68)
                        : "No Title Available"
                    }
                    description={
                      element.description
                        ? element.description?.slice(0, 90)
                        : "No Description available for this news. Please click on Read More buttion to read full news"
                    } //slice method will set how many characters we want to display
